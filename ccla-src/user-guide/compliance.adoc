...
When managing infrastructures for production use cases, security and compliance are crucial factors to consider. The clusters need to be securely configured to minimize the attack surface. Additionally, each organization may have audit requirements for systems to be compliant with security standards.
...
We have reutilized the concept of Implementation Groups (IG) to drive how deep an environment is affected by the compliance needs.
We assume the level of compliance, depending on your environment type, therefore, the associated security rules are not same if you deploy a local sandbox, a pre-production or a production platform.
...
* *Extended* : controls focused on helping security teams manage sensitive clients
...
From the environment home page, a user can access the compliance dashboard by clicking on *Compliance*

Depending on the standards selected in environment settings, each row represents a percentage of completion of standards with 3 colors 
...
After running a compliance scan, for each standard, a JSON report is available in the dashboard. Click on the arrow to download it
...
Depending on the type of K8 cluster, a CIS benchmarks report can be downloaded to find details of the test and remediation mechanism
...
Like many automation tasks in Cloudclapp, the compliance scan is implemented via a worklow
